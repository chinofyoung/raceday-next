...
function EventFormContent({ initialData, isEditing, draftId, setDraftId, loading, setLoading }: EventFormContentProps) {
    const { user } = useAuth();
    const router = useRouter();
    const { handleSubmit, control, getValues } = useFormContext<EventFormInput>();
    const [maxStepReached, setMaxStepReached] = useState(isEditing ? STEPS.length - 1 : 0);

    const createEvent = useMutation(api.events.create);
    const updateEvent = useMutation(api.events.update);

    const { currentStep, setCurrentStep, nextStep: baseNextStep, prevStep } = useFormSteps<EventFormInput>(
        STEPS.length,
        STEP_FIELDS as any
    );
    ...
    }, [user, draftId, getValues, createEvent, updateEvent, setDraftId]);
