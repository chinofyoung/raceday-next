    ...
    return (
        <div id="route" className="space-y-8 px-4 md:px-0 relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <h2 className="text-3xl font-bold tracking-tight text-white">Race <span className="text-primary">Course</span></h2>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {event.categories.map((cat, i) => (
                            <button
                                ...
                                className={cn(
                                    "px-4 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all border",
            ...
            {event.categories?.[activeRouteCategoryIndex]?.routeMap?.gpxFileUrl ? (
                <div key={activeRouteCategoryIndex} className="space-y-6 animate-in fade-in duration-500">
                    <div className="aspect-square md:aspect-[16/9] w-full rounded-2xl overflow-hidden border border-white/5 shadow-2xl relative isolate">
                        {(() => {
