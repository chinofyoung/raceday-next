        ...
        <Card className="p-6 bg-white/5 border-white/10 relative overflow-hidden flex flex-col h-full">
            ...
            <div className="flex items-center justify-between mb-6 relative z-10 w-full">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-base font-black uppercase italic tracking-tight text-white hidden sm:block">Revenue</h3>
                </div>
                <div className="flex bg-black/40 p-1 rounded-xl border border-white/10 shrink-0">
                    <button
                        onClick={() => setView("category")}
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-black italic uppercase transition-all ${view === "category"
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-black italic uppercase transition-all ${view === "event"
                            ? "bg-white/10 text-white shadow-sm"
                            : "text-text-muted hover:text-white"
                            }`}
                    >
                    ...
                    <div key={item.id || item.name} className="space-y-2 group">
                        <div className="flex items-center justify-between">
                            <div className="flex flex-col min-w-0 mr-4">
                                <span className="text-sm font-bold text-white uppercase italic truncate group-hover:text-green-400 transition-colors">{item.name}</span>
                                {item.eventInfo && (
                                    <span className="text-[10px] text-text-muted font-bold italic truncate">{item.eventInfo}</span>
                                )}
                            </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] text-text-muted font-bold italic w-16 text-right">{item.count} runner{item.count !== 1 && 's'}</span>
                        </div>
                    </div>
                ))}
            </div>
            <div className="mt-6 pt-4 border-t border-white/10 relative z-10">
                <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-muted italic">Total Revenue</span>
                    <span className="text-xl font-black text-white italic drop-shadow-md">₱{totalRevenue.toLocaleString()}</span>
                </div>
            </div>
        </Card>
    );
