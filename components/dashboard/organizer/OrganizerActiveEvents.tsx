...
function OrganizerActiveEventsComponent({ items, eventKitStats }: OrganizerActiveEventsProps) {
    ...
    return (
        <Card className="p-6 bg-white/5 border-white/10 relative group flex flex-col h-full overflow-hidden">
            ...
            {/* Event list */}
            <div className="flex flex-col gap-3 relative z-10 flex-1 overflow-y-auto min-h-0 custom-scrollbar pr-1">
                {eventKitStats.map((event) => {
                    ...
                    return (
                        <div
                            key={event.id}
                            className="group/card relative flex gap-4 bg-black/20 hover:bg-white/5 border border-white/5 hover:border-cta/20 rounded-2xl p-3 transition-all duration-300 overflow-hidden shrink-0 h-[100px]"
                        >
                            {/* Subtle hover glow */}
                            ...
                            {/* Square image — matches card height */}
                            <div className="relative aspect-square h-full rounded-xl bg-white/5 border border-white/10 overflow-hidden shrink-0 shadow-md">
                                {event.featuredImage ? (
                                    <Image
                            ...
                            <div className="flex-1 min-w-0 flex flex-col justify-between">
                                ...
                                {/* Stats + actions */}
                                <div className="flex flex-wrap items-center gap-3 relative z-10 pointer-events-auto">
